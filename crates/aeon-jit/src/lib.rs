...
mod tests {
    ...
    #[test]
    fn expr_bit_reverse_operation() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Rev(Box::new(Expr::Imm(0x0102030405060708))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[1], 0x0807060504030201);
    }

    #[test]
    fn expr_shift_left_by_immediate() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Shl(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[1], 256);
    }

    #[test]
    fn expr_multiply_large_values() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::Mul(
                Box::new(Expr::Imm(0x100000)),
                Box::new(Expr::Imm(0x100000)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[2], 0x10000000000);
    }

    #[test]
    fn expr_long_chain_of_operations() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        // ((((1 + 2) * 3) - 4) >> 1) & 0x0F
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(5),
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // ((1 + 2) * 3 - 4) >> 1 & 0x0F = (9 - 4) >> 1 & 0x0F = 5 >> 1 & 0x0F = 2 & 0x0F = 2
        assert_eq!(ctx.x[5], 2);
    }

    #[test]
    fn stmt_pair_with_conditionals() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::SetFlags {
                expr: Expr::Sub(
    ...
    #[test]
    fn expr_xor_self_clears() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Xor(
                Box::new(Expr::Imm(0x1234)),
                Box::new(Expr::Imm(0x1234)),
            ),
        }];
    ...
    #[test]
    fn expr_insert_into_high_bits() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::Insert {
                dst: Box::new(Expr::Imm(0)),
                src: Box::new(Expr::Imm(0x1234)),
                lsb: 48,
                width: 16,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[2], 0x1234000000000000);
    }

    #[test]
    fn expr_all_registers_preserved() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::Assign {
                dst: Reg::X(0),
        ...
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[0], 1);
        assert_eq!(ctx.x[15], 0xF000);
        assert_eq!(ctx.x[30], 0x1E00);
    }

    #[test]
    fn expr_register_to_register_copy() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::Assign {
                dst: Reg::X(0),
                src: Expr::Imm(0x123456789ABCDEF0),
            },
            Stmt::Assign {
                dst: Reg::X(1),
                src: Expr::Reg(Reg::X(0)),
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[0], 0x123456789ABCDEF0);
        assert_eq!(ctx.x[1], 0x123456789ABCDEF0);
        assert_eq!(ctx.x[2], 0x123456789ABCDEF0);
    }

    #[test]
    fn expr_constant_folding_opportunities() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Add(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[1], 120);
    }

    #[test]
    fn expr_zero_extend_preserves_upper() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::ZeroExtend {
                src: Box::new(Expr::Imm(0x000000FF)),
                from_bits: 8,
            },
        }];
    ...
    #[test]
    fn expr_or_with_zero_identity() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Or(
                Box::new(Expr::Imm(0xDEADBEEF)),
                Box::new(Expr::Imm(0)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[1], 0xDEADBEEF);
    }

    #[test]
    fn expr_and_with_all_ones() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::And(
                Box::new(Expr::Imm(0x12345678)),
                Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[1], 0x12345678);
    }

    #[test]
    fn expr_divide_by_one_identity() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::UDiv(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[2], 0xDEADBEEF);
    }

    #[test]
    fn expr_rotate_full_circle() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::Assign {
                dst: Reg::X(1),
                src: Expr::Ror(
                    Box::new(Expr::Imm(0x1234567890ABCDEF)),
                    Box::new(Expr::Imm(64)),
                ),
            },
        ];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // Rotating by 64 should return to original (modulo 64)
        assert_eq!(ctx.x[1], 0x1234567890ABCDEF);
    }

    #[test]
    fn expr_multiple_negations() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Neg(Box::new(Expr::Neg(Box::new(Expr::Imm(42))))),
        }];
    ...
    #[test]
    fn expr_add_with_overflow_wrapping() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::Add(
    ...
    #[test]
    fn expr_extract_at_boundary() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF)),
                lsb: 0,
                width: 64,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[1], 0xFFFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_fsqrt_floating_point_square_root() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(0),
            src: Expr::FSqrt(Box::new(Expr::FImm(4.0))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // Verify FSqrt compiles successfully
        let _ = ctx.x[0];
    }

    #[test]
    fn expr_fmax_floating_point_maximum() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::FMax(
                Box::new(Expr::FImm(3.14)),
                Box::new(Expr::FImm(2.71)),
            ),
        }];
    ...
    #[test]
    fn expr_fmin_floating_point_minimum() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::FMin(
                Box::new(Expr::FImm(1.41)),
                Box::new(Expr::FImm(1.73)),
            ),
        }];
    ...
    #[test]
    fn expr_adrp_immediate_address() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(3),
            src: Expr::AdrpImm(0x400000),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[3], 0x400000);
    }

    #[test]
    fn expr_adr_immediate_address() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(4),
            src: Expr::AdrImm(0x500000),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[4], 0x500000);
    }

    #[test]
    fn expr_chained_float_operations() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(3),
            src: Expr::FMin(
                Box::new(Expr::FSqrt(Box::new(Expr::FImm(16.0)))),
                Box::new(Expr::FMax(
                    Box::new(Expr::FImm(3.0)),
                    Box::new(Expr::FImm(5.0)),
                )),
            ),
        }];
    ...
    #[test]
    fn expr_address_arithmetic_adrp_plus_imm() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::Assign {
                dst: Reg::X(5),
                src: Expr::AdrpImm(0x1000),
            },
            Stmt::Assign {
                dst: Reg::X(6),
                src: Expr::Add(
                    Box::new(Expr::Reg(Reg::X(5))),
                    Box::new(Expr::Imm(0x100)),
                ),
            },
        ];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[5], 0x1000);
        assert_eq!(ctx.x[6], 0x1100);
    }

    #[test]
    fn expr_int_to_float_conversion() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(0),
            src: Expr::IntToFloat(Box::new(Expr::Imm(42))),
        }];
    ...
    #[test]
    fn expr_float_to_int_conversion() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::FloatToInt(Box::new(Expr::FImm(3.14))),
        }];
    ...
    #[test]
    fn expr_int_to_float_then_back() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::FloatToInt(Box::new(Expr::IntToFloat(Box::new(Expr::Imm(100))))),
        }];
    ...
    #[test]
    fn expr_fneg_floating_negate() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(3),
            src: Expr::FNeg(Box::new(Expr::FImm(2.5))),
        }];
    ...
    #[test]
    fn expr_fabs_floating_absolute() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(4),
            src: Expr::FAbs(Box::new(Expr::FImm(-3.5))),
        }];
    ...
    #[test]
    fn expr_fadd_float_addition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(5),
            src: Expr::FAdd(
                Box::new(Expr::FImm(1.5)),
                Box::new(Expr::FImm(2.5)),
            ),
        }];
    ...
    #[test]
    fn expr_fsub_float_subtraction() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(6),
            src: Expr::FSub(
                Box::new(Expr::FImm(5.0)),
                Box::new(Expr::FImm(2.0)),
            ),
        }];
    ...
    #[test]
    fn expr_fmul_float_multiplication() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(7),
            src: Expr::FMul(
    ...
    #[test]
    fn expr_fdiv_float_division() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(8),
            src: Expr::FDiv(
                Box::new(Expr::FImm(10.0)),
                Box::new(Expr::FImm(2.0)),
            ),
        }];
    ...
    #[test]
    fn expr_complex_float_arithmetic_chain() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(9),
            src: Expr::FDiv(
    ...
    #[test]
    fn stmt_setflags_with_float_operation() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::SetFlags {
            expr: Expr::FSub(
                Box::new(Expr::FImm(5.0)),
                Box::new(Expr::FImm(3.0)),
            ),
    ...
    #[test]
    fn expr_compare_signed_values() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(10),
            src: Expr::Compare {
                cond: Condition::EQ,
                lhs: Box::new(Expr::Imm(100)),
                rhs: Box::new(Expr::Imm(100)),
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[10], 1);
    }

    #[test]
    fn expr_compare_not_equal() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(11),
            src: Expr::Compare {
                cond: Condition::NE,
                lhs: Box::new(Expr::Imm(50)),
                rhs: Box::new(Expr::Imm(100)),
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[11], 1);
    }

    #[test]
    fn expr_compare_less_than_signed() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(12),
            src: Expr::Compare {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[12], 1);
    }

    #[test]
    fn expr_compare_greater_than_or_equal() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(13),
            src: Expr::Compare {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[13], 1);
    }

    #[test]
    fn expr_compare_unsigned_greater() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(14),
            src: Expr::Compare {
                cond: Condition::HI,
                lhs: Box::new(Expr::Imm(200)),
                rhs: Box::new(Expr::Imm(100)),
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[14], 1);
    }

    #[test]
    fn expr_cond_select_with_compare() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(15),
            src: Expr::CondSelect {
                cond: Condition::LT,
                if_true: Box::new(Expr::Imm(10)),
                if_false: Box::new(Expr::Imm(20)),
            },
        }];
    ...
    #[test]
    fn expr_fcvt_floating_convert() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(16),
            src: Expr::FCvt(Box::new(Expr::FImm(3.14159))),
        }];
    ...
    #[test]
    fn expr_insert_into_register() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(17),
            src: Expr::Insert {
                dst: Box::new(Expr::Imm(0x00FF00FF)),
                src: Box::new(Expr::Imm(0xAB)),
                lsb: 8,
                width: 8,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[17], 0x00FFABFF);
    }

    #[test]
    fn expr_multiple_mixed_operations() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(18),
            src: Expr::Or(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // 0x0F << 4 = 0xF0, 0xFF & 0x0F = 0x0F, 0xF0 | 0x0F = 0xFF
        assert_eq!(ctx.x[18], 0xFF);
    }

    #[test]
    fn expr_deeply_nested_arithmetic() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(19),
            src: Expr::Add(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // (10 - 3) * 2 + 16 / 4 = 7 * 2 + 4 = 14 + 4 = 18
        assert_eq!(ctx.x[19], 18);
    }

    #[test]
    fn expr_zero_width_extract_edge_case() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(20),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0x12345678)),
                lsb: 16,
                width: 1,
            },
        }];
    ...
    #[test]
    fn expr_sign_extend_negative_value() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(21),
            src: Expr::SignExtend {
                src: Box::new(Expr::Imm(0xFF)),
                from_bits: 8,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // Sign extend 0xFF (8-bit) should give 0xFFFFFFFFFFFFFFFF
        assert_eq!(ctx.x[21], 0xFFFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_zero_extend_short_value() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(22),
            src: Expr::ZeroExtend {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[22], 0xFF);
    }

    #[test]
    fn expr_boolean_logic_chains() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(23),
            src: Expr::And(
                Box::new(Expr::Or(
                    Box::new(Expr::Imm(0x0F)),
                    Box::new(Expr::Imm(0xF0)),
                )),
                Box::new(Expr::Not(Box::new(Expr::Imm(0x00)))),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // (0x0F | 0xF0) & ~0x00 = 0xFF & 0xFFFFFFFFFFFFFFFF = 0xFF
        assert_eq!(ctx.x[23], 0xFF);
    }

    #[test]
    fn expr_asr_with_negative_high_bit() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(24),
            src: Expr::Asr(
                Box::new(Expr::Imm(0x8000000000000000)),
                Box::new(Expr::Imm(32)),
            ),
        }];
    ...
    #[test]
    fn expr_lsr_mask_pattern() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(25),
            src: Expr::Lsr(
                Box::new(Expr::Imm(0xFFFFFFFF00000000)),
                Box::new(Expr::Imm(16)),
            ),
        }];
    ...
    #[test]
    fn expr_compare_greater_than() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(26),
            src: Expr::Compare {
                cond: Condition::GT,
                lhs: Box::new(Expr::Imm(150)),
                rhs: Box::new(Expr::Imm(100)),
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[26], 1);
    }

    #[test]
    fn expr_compare_less_than_or_equal() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(27),
            src: Expr::Compare {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[27], 1);
    }

    #[test]
    fn expr_compare_unsigned_less_or_equal() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(28),
            src: Expr::Compare {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[28], 1);
    }

    #[test]
    fn expr_abs_of_negative() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(29),
            src: Expr::Abs(Box::new(Expr::Imm((-50i64) as u64))),
        }];
    ...
    #[test]
    fn expr_abs_of_positive() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(30),
            src: Expr::Abs(Box::new(Expr::Imm(42))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[30], 42);
    }

    #[test]
    fn expr_xor_bitwise_toggle() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(0),
            src: Expr::Xor(
                Box::new(Expr::Imm(0xAAAAAAAAAAAAAAAA)),
                Box::new(Expr::Imm(0x5555555555555555)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[0], 0xFFFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_div_by_larger_divisor() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Div(
    ...
    #[test]
    fn expr_mul_large_values() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::Mul(
                Box::new(Expr::Imm(0x100000000)),
                Box::new(Expr::Imm(0x100000000)),
            ),
        }];
    ...
    fn expr_sub_with_borrow() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // 10 - 20 = -10, wraps to large positive
        let _ = ctx.x[3];
    }

    #[test]
    fn expr_compare_always_true() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(30),
            src: Expr::Compare {
                cond: Condition::AL,
                lhs: Box::new(Expr::Imm(0)),
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[30], 1);
    }

    #[test]
    fn expr_compare_never_true() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(0),
            src: Expr::Compare {
                cond: Condition::NV,
                lhs: Box::new(Expr::Imm(1)),
                rhs: Box::new(Expr::Imm(1)),
            },
        }];
    ...
    #[test]
    fn expr_clz_leading_zeros() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::Clz(Box::new(Expr::Imm(0x0000000000000001))),
        }];
    ...
    #[test]
    fn expr_clz_no_zeros() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::Clz(Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF))),
        }];
    ...
    #[test]
    fn expr_pair_simple_sequence() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Pair(
            Box::new(Stmt::Assign {
                dst: Reg::X(3),
                src: Expr::Imm(100),
            }),
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[3], 100);
        assert_eq!(ctx.x[4], 150);
    }

    #[test]
    fn expr_fsub_float_subtraction_chain() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(5),
            src: Expr::FSub(
    ...
    #[test]
    fn expr_mixed_int_float_operations() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(6),
            src: Expr::Add(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[6], 30);
    }

    #[test]
    fn expr_extract_middle_bits() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(7),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0x0000FFFF0000FFFF)),
                lsb: 16,
                width: 16,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[7], 0x0000);
    }

    #[test]
    fn expr_insert_full_word() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(8),
            src: Expr::Insert {
                dst: Box::new(Expr::Imm(0xDEADBEEF00000000)),
                src: Box::new(Expr::Imm(0xCAFEBABE)),
                lsb: 0,
                width: 32,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[8], 0xDEADBEEFCAFEBABE);
    }

    #[test]
    fn expr_three_level_nesting() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(9),
            src: Expr::Mul(
        ...
        unsafe { func(&mut ctx) };
        // (2 + 3) * (10 - 4) = 5 * 6 = 30
        assert_eq!(ctx.x[9], 30);
    }

    #[test]
    fn expr_four_level_bit_operations() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(10),
            src: Expr::And(
                Box::new(Expr::Or(
                    Box::new(Expr::Imm(0x00FF0000)),
                    Box::new(Expr::Imm(0xFF000000)),
                )),
                Box::new(Expr::Not(
                    Box::new(Expr::Imm(0x0000FF00)),
                )),
            ),
        }];
    ...
    #[test]
    fn expr_floating_max_chain() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(11),
            src: Expr::FMax(
                Box::new(Expr::FImm(1.0)),
                Box::new(Expr::FMax(
                    Box::new(Expr::FImm(2.0)),
                    Box::new(Expr::FImm(3.0)),
                )),
    ...
    #[test]
    fn expr_compare_unsigned_carry() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(12),
            src: Expr::Compare {
                cond: Condition::CS,
                lhs: Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF)),
                rhs: Box::new(Expr::Imm(1)),
            },
        }];
    ...
    #[test]
    fn expr_compare_unsigned_no_carry() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(13),
            src: Expr::Compare {
                cond: Condition::CC,
                lhs: Box::new(Expr::Imm(10)),
                rhs: Box::new(Expr::Imm(5)),
            },
        }];
    ...
    #[test]
    fn expr_compare_signed_overflow() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(14),
            src: Expr::Compare {
    ...
    #[test]
    fn expr_compare_signed_no_overflow() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(15),
            src: Expr::Compare {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[15], 1);
    }

    #[test]
    fn expr_shl_then_extract() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(16),
            src: Expr::Extract {
                src: Box::new(Expr::Shl(
                    Box::new(Expr::Imm(0x00FF)),
                    Box::new(Expr::Imm(8)),
                )),
                lsb: 8,
                width: 8,
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[16], 0xFF);
    }

    #[test]
    fn expr_extract_then_insert() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(17),
            src: Expr::Insert {
                dst: Box::new(Expr::Imm(0x00000000)),
                src: Box::new(Expr::Extract {
                    src: Box::new(Expr::Imm(0xFFFFFFFF)),
                    lsb: 8,
                    width: 8,
                }),
                lsb: 16,
    ...
    fn expr_shl_32_position() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[18], 0x100000000);
    }

    #[test]
    fn expr_lsr_32_position() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(19),
            src: Expr::Lsr(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[19], 1);
    }

    #[test]
    fn expr_ror_various_amounts() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(20),
            src: Expr::Ror(
    ...
    #[test]
    fn expr_max_boundary_add() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(21),
            src: Expr::Add(
                Box::new(Expr::Imm(0x7FFFFFFFFFFFFFFF)),
                Box::new(Expr::Imm(1)),
            ),
        }];
    ...
    #[test]
    fn expr_zero_division_identity() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(22),
            src: Expr::UDiv(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[22], 0);
    }

    #[test]
    fn expr_all_ones_operations() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(23),
            src: Expr::Neg(Box::new(Expr::Imm(1))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[23], 0xFFFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_floating_add_chain() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(24),
            src: Expr::FAdd(
    ...
    #[test]
    fn expr_cls_counting_sign_bits() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(26),
            src: Expr::Cls(Box::new(Expr::Imm(0x0000000000000001))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[26], 62);
    }

    #[test]
    fn expr_compare_positive_vs_negative() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(27),
            src: Expr::Compare {
    ...
    #[test]
    fn expr_mixed_shift_and_logical() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(5),
            src: Expr::And(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[5], 0x0000FF00);
    }

    #[test]
    fn expr_extract_with_extend() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(6),
            src: Expr::ZeroExtend {
                src: Box::new(Expr::Extract {
                    src: Box::new(Expr::Imm(0xFF00)),
                    lsb: 8,
                    width: 8,
                }),
                from_bits: 8,
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[6], 0xFF);
    }

    #[test]
    fn expr_floating_to_int_and_back() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(7),
            src: Expr::IntToFloat(
                Box::new(Expr::FloatToInt(
                    Box::new(Expr::FImm(42.5))
                ))
            ),
        }];
    ...
    fn expr_alternating_and_or() {
        ...
        let stmts = vec![Stmt::Assign {
            ...
            src: Expr::Or(
                Box::new(Expr::And(
                    Box::new(Expr::Imm(0xAAAAAAAAAAAAAAAA)),
                    Box::new(Expr::Imm(0xCCCCCCCCCCCCCCCC)),
                )),
                Box::new(Expr::And(
                    Box::new(Expr::Imm(0x5555555555555555)),
                    Box::new(Expr::Imm(0x3333333333333333)),
                )),
            ),
        }];
    ...
    #[test]
    fn expr_double_extract_chain() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(9),
            src: Expr::Extract {
                src: Box::new(Expr::Extract {
                    src: Box::new(Expr::Imm(0xFF00FF00FF00FF00)),
                    lsb: 8,
                    width: 16,
                }),
                lsb: 4,
    ...
    #[test]
    fn expr_large_multiplication() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(10),
            src: Expr::Mul(
                Box::new(Expr::Imm(0x1000000000000000)),
                Box::new(Expr::Imm(0x0000000000000010)),
            ),
        }];
    ...
    #[test]
    fn expr_compare_boundary_eq() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(11),
            src: Expr::Compare {
                cond: Condition::EQ,
                lhs: Box::new(Expr::Imm(0x8000000000000000)),
                rhs: Box::new(Expr::Imm(0x8000000000000000)),
            },
        }];
    ...
    #[test]
    fn expr_floating_div_by_two() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(13),
            src: Expr::FDiv(
                Box::new(Expr::FImm(100.0)),
                Box::new(Expr::FImm(2.0)),
            ),
        }];
    ...
    #[test]
    fn expr_insert_multiple_bytes() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(14),
            src: Expr::Insert {
                dst: Box::new(Expr::Imm(0x0000000000000000)),
                src: Box::new(Expr::Imm(0x1234567890ABCDEF)),
                lsb: 0,
                width: 64,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[14], 0x1234567890ABCDEF);
    }

    #[test]
    fn expr_extract_all_bits() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(15),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0xDEADBEEFCAFEBABE)),
                lsb: 0,
                width: 64,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[15], 0xDEADBEEFCAFEBABE);
    }
    ...
    fn expr_shl_63_bits() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[16], 0x8000000000000000);
    }
    ...
    fn expr_ror_by_1() {
        ...
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(17),
            src: Expr::Ror(
                Box::new(Expr::Imm(0x0000000000000001)),
                Box::new(Expr::Imm(1)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[17], 0x8000000000000000);
    }

    #[test]
    fn expr_clz_all_ones() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(18),
            src: Expr::Clz(Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF))),
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[18], 0);
    }

    #[test]
    fn expr_sign_extend_32bit() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(19),
            src: Expr::SignExtend {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[19], 0xFFFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_zero_extend_32bit() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(20),
            src: Expr::ZeroExtend {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[20], 0xFFFFFFFF);
    }

    #[test]
    fn expr_div_identity_self() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(21),
            src: Expr::Div(
                Box::new(Expr::Imm(1000)),
                Box::new(Expr::Imm(1000)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[21], 1);
    }

    #[test]
    fn expr_udiv_identity_self() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(22),
            src: Expr::UDiv(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[22], 1);
    }

    #[test]
    fn expr_fsqrt_perfect_square() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(23),
            src: Expr::FSqrt(Box::new(Expr::FImm(9.0))),
        }];
    ...
    #[test]
    fn expr_not_inverts_all() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(26),
            src: Expr::Not(Box::new(Expr::Imm(0))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[26], 0xFFFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_not_clears_all() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(27),
            src: Expr::Not(Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[27], 0);
    }

    #[test]
    fn expr_compare_all_conditions_with_equal() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::Assign {
                dst: Reg::X(0),
                src: Expr::Compare {
                    cond: Condition::EQ,
                    lhs: Box::new(Expr::Imm(42)),
                    rhs: Box::new(Expr::Imm(42)),
                },
            },
            Stmt::Assign {
                dst: Reg::X(1),
    ...
    fn expr_asr_by_zero() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[2], 0x8000000000000000);
    }
    ...
    fn expr_ror_by_zero() {
        ...
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(3),
            src: Expr::Ror(
                Box::new(Expr::Imm(0xDEADBEEFCAFEBABE)),
                Box::new(Expr::Imm(0)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[3], 0xDEADBEEFCAFEBABE);
    }

    #[test]
    fn stmt_ten_register_accumulation() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::Assign {
                dst: Reg::X(0),
    ...
    #[test]
    fn expr_floating_abs_negative() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(5),
            src: Expr::FAbs(Box::new(Expr::FImm(-10.5))),
        }];
    ...
    #[test]
    fn expr_floating_negate_zero() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(7),
            src: Expr::FNeg(Box::new(Expr::FImm(0.0))),
        }];
    ...
    #[test]
    fn expr_extract_single_bit() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(8),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0x0000000000000001)),
                lsb: 0,
                width: 1,
            },
        }];
    ...
    #[test]
    fn expr_extract_single_zero_bit() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(9),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFE)),
                lsb: 0,
                width: 1,
            },
        }];
    ...
    #[test]
    fn expr_insert_single_bit() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(10),
            src: Expr::Insert {
                dst: Box::new(Expr::Imm(0x0000000000000000)),
                src: Box::new(Expr::Imm(1)),
                lsb: 63,
                width: 1,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[10], 0x8000000000000000);
    }

    #[test]
    fn expr_xor_same_value_clears() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(11),
            src: Expr::Xor(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[11], 0);
    }

    #[test]
    fn expr_and_same_preserves() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(12),
            src: Expr::And(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[12], 0x1234567890ABCDEF);
    }

    #[test]
    fn expr_or_self_preserves() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(13),
            src: Expr::Or(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[13], 0xAAAAAAAAAAAAAAAA);
    }

    #[test]
    fn expr_multiply_by_zero() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(14),
            src: Expr::Mul(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[14], 0);
    }

    #[test]
    fn expr_multiply_by_one() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(15),
            src: Expr::Mul(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[15], 0x1234567890ABCDEF);
    }
    ...
    fn expr_sub_zero() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[16], 0x1234567890ABCDEF);
    }
    ...
    fn expr_add_zero() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[17], 0x1234567890ABCDEF);
    }

    #[test]
    fn expr_floating_add_zero() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(18),
            src: Expr::FAdd(
                Box::new(Expr::FImm(42.0)),
                Box::new(Expr::FImm(0.0)),
            ),
        }];
    ...
    #[test]
    fn expr_cls_all_zeros() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(21),
            src: Expr::Cls(Box::new(Expr::Imm(0))),
        }];
    ...
    #[test]
    fn expr_rbit_bit_reverse() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(23),
            src: Expr::Rbit(Box::new(Expr::Imm(0x8000000000000000))),
        }];
    ...
    #[test]
    fn expr_extract_msb_32bits() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(24),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0xFFFFFFFF00000000)),
                lsb: 32,
                width: 32,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[24], 0xFFFFFFFF);
    }

    #[test]
    fn expr_extract_lsb_32bits() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(25),
            src: Expr::Extract {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[25], 0);
    }

    #[test]
    fn expr_sign_extend_16bit() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(26),
            src: Expr::SignExtend {
                src: Box::new(Expr::Imm(0x8000)),
                from_bits: 16,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[26], 0xFFFFFFFFFFFF8000);
    }

    #[test]
    fn expr_zero_extend_16bit() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(27),
            src: Expr::ZeroExtend {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[27], 0x8000);
    }

    #[test]
    fn expr_chained_shift_operations() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(28),
            src: Expr::Lsr(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        // (0xFF << 8) = 0xFF00, 0xFF00 >> 4 = 0xFF0
        assert_eq!(ctx.x[28], 0xFF0);
    }

    #[test]
    fn expr_floating_operations_nested_add_mul() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(0),
            src: Expr::FAdd(
    ...
    #[test]
    fn expr_floating_subtract_then_multiply() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::FMul(
    ...
    #[test]
    fn expr_extract_insert_roundtrip() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::Insert {
                dst: Box::new(Expr::Extract {
                    src: Box::new(Expr::Imm(0xAAAAAAAAAAAAAAAA)),
                    lsb: 0,
                    width: 64,
                }),
                src: Box::new(Expr::Imm(0x5555555555555555)),
                lsb: 0,
                width: 32,
            },
        }];
    ...
    #[test]
    fn expr_cond_select_with_gt_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(3),
            src: Expr::CondSelect {
    ...
    #[test]
    fn expr_cond_select_with_hs_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(4),
            src: Expr::CondSelect {
                cond: Condition::HI,
                if_true: Box::new(Expr::Imm(200)),
                if_false: Box::new(Expr::Imm(100)),
            },
        }];
    ...
    #[test]
    fn expr_multiple_extracts_different_widths() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::Assign {
                dst: Reg::X(5),
        ...
        assert_eq!(ctx.x[6], 0xFFFF);
        assert_eq!(ctx.x[7], 0xFFFFFFFF);
    }

    #[test]
    fn expr_clz_with_power_of_two() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(8),
            src: Expr::Clz(Box::new(Expr::Imm(0x0100000000000000))),
        }];
    ...
    #[test]
    fn expr_compare_with_max_values() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(9),
            src: Expr::Compare {
                cond: Condition::EQ,
                lhs: Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF)),
                rhs: Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF)),
            },
        }];
    ...
    #[test]
    fn expr_compare_with_min_values() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(10),
            src: Expr::Compare {
    ...
    #[test]
    fn expr_sign_extend_preserve_positive() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(11),
            src: Expr::SignExtend {
                src: Box::new(Expr::Imm(0x7F)),
                from_bits: 8,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[11], 0x7F);
    }

    #[test]
    fn expr_zero_extend_preserves_value() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(12),
            src: Expr::ZeroExtend {
    ...
    fn expr_asr_by_16() {
        ...
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(16),
            src: Expr::Asr(
                Box::new(Expr::Imm(0xFFFF0000FFFF0000)),
                Box::new(Expr::Imm(16)),
            ),
        }];
    ...
    fn expr_ror_by_48() {
        ...
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(17),
            src: Expr::Ror(
                Box::new(Expr::Imm(0x123456789ABCDEF0)),
                Box::new(Expr::Imm(48)),
            ),
        }];
    ...
    #[test]
    fn expr_complex_boolean_with_shift() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(18),
            src: Expr::And(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[18], 0xFF000000);
    }

    #[test]
    fn expr_floating_div_by_four() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(19),
            src: Expr::FDiv(
                Box::new(Expr::FImm(8.0)),
                Box::new(Expr::FImm(4.0)),
            ),
        }];
    ...
    #[test]
    fn expr_insert_high_bytes() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(20),
            src: Expr::Insert {
                dst: Box::new(Expr::Imm(0x00000000FFFFFFFF)),
                src: Box::new(Expr::Imm(0xDEADBEEF)),
                lsb: 32,
                width: 32,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[20], 0xDEADBEEFFFFFFFFF);
    }

    #[test]
    fn expr_extract_middle_dword() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(21),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0x0102030405060708)),
                lsb: 16,
                width: 32,
            },
        }];
    ...
    #[test]
    fn expr_neg_zero_unchanged() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(22),
            src: Expr::Neg(Box::new(Expr::Imm(0))),
        }];
    ...
    #[test]
    fn expr_abs_zero_unchanged() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(23),
            src: Expr::Abs(Box::new(Expr::Imm(0))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[23], 0);
    }

    #[test]
    fn expr_floating_mul_zero() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(24),
            src: Expr::FMul(
    ...
    #[test]
    fn expr_invert_then_invert() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(26),
            src: Expr::Not(Box::new(Expr::Not(Box::new(Expr::Imm(0x1234567890ABCDEF))))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[26], 0x1234567890ABCDEF);
    }

    #[test]
    fn expr_and_complement_clears() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(27),
            src: Expr::And(
                Box::new(Expr::Imm(0xAAAAAAAAAAAAAAAA)),
                Box::new(Expr::Not(Box::new(Expr::Imm(0xAAAAAAAAAAAAAAAA)))),
            ),
        }];
    ...
    #[test]
    fn expr_or_complement_sets_all() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(28),
            src: Expr::Or(
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[28], 0xFFFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_compare_unsigned_boundary() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(29),
            src: Expr::Compare {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[29], 1);
    }

    #[test]
    fn expr_cond_select_with_vc_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(30),
            src: Expr::CondSelect {
    ...
    #[test]
    fn expr_cond_select_with_vs_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(0),
            src: Expr::CondSelect {
    ...
    #[test]
    fn expr_cond_select_with_ls_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(1),
            src: Expr::CondSelect {
    ...
    #[test]
    fn expr_nested_extract_insert() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(2),
            src: Expr::Insert {
                dst: Box::new(Expr::Imm(0x1111111111111111)),
                src: Box::new(Expr::Extract {
                    src: Box::new(Expr::Imm(0x2222222222222222)),
                    lsb: 8,
                    width: 16,
                }),
                lsb: 32,
    ...
    fn expr_shift_left_by_one() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[3], 0x0000000000000002);
    }

    #[test]
    fn expr_shift_right_by_one() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(4),
            src: Expr::Lsr(
                Box::new(Expr::Imm(0x0000000000000002)),
                Box::new(Expr::Imm(1)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[4], 0x0000000000000001);
    }

    #[test]
    fn expr_abs_of_max_signed() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(5),
            src: Expr::Abs(Box::new(Expr::Imm(0x7FFFFFFFFFFFFFFF))),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[5], 0x7FFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_extract_lower_nybble() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(6),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0xFFFFFFFFFFFFFFAB)),
                lsb: 0,
                width: 4,
            },
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[6], 0xB);
    }

    #[test]
    fn expr_extract_upper_nybble() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(7),
            src: Expr::Extract {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[7], 0xA);
    }

    #[test]
    fn expr_floating_sqrt_one() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(8),
            src: Expr::FSqrt(Box::new(Expr::FImm(1.0))),
        }];
    ...
    #[test]
    fn expr_cond_select_with_cs_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(12),
            src: Expr::CondSelect {
    ...
    #[test]
    fn expr_cond_select_with_cc_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(13),
            src: Expr::CondSelect {
    ...
    #[test]
    fn expr_cond_select_with_al_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(14),
            src: Expr::CondSelect {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[14], 100);
    }

    #[test]
    fn expr_cond_select_with_nv_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(15),
            src: Expr::CondSelect {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[15], 0);
    }

    #[test]
    fn expr_multiple_operations_sequence() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![
            Stmt::Assign {
                dst: Reg::X(0),
    ...
    #[test]
    fn expr_div_by_power_of_two() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(4),
            src: Expr::UDiv(
                Box::new(Expr::Imm(256)),
                Box::new(Expr::Imm(4)),
            ),
        }];
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[4], 64);
    }

    #[test]
    fn expr_floating_operations_chain_five() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(5),
            src: Expr::FAdd(
    ...
    #[test]
    fn expr_extract_high_byte() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(6),
            src: Expr::Extract {
                src: Box::new(Expr::Imm(0xFF00000000000000)),
                lsb: 56,
                width: 8,
            },
        }];
    ...
    fn expr_insert_single_byte() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[7], 0xAB00);
    }

    #[test]
    fn expr_compare_all_zeros() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(8),
            src: Expr::Compare {
    ...
    #[test]
    fn expr_compare_different_zeros() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(9),
            src: Expr::Compare {
    ...
    #[test]
    fn expr_cond_select_with_mi_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(10),
            src: Expr::CondSelect {
    ...
    #[test]
    fn expr_cond_select_with_pl_condition() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(11),
            src: Expr::CondSelect {
    ...
    fn expr_xor_alternating_bits() {
        ...
        let mut ctx = JitContext::default();
        unsafe { func(&mut ctx) };
        assert_eq!(ctx.x[12], 0xFFFFFFFFFFFFFFFF);
    }

    #[test]
    fn expr_and_clear_high_bits() {
        let mut compiler = JitCompiler::new(JitConfig::default());
        let stmts = vec![Stmt::Assign {
            dst: Reg::X(13),
            src: Expr::And(
                Box::new(Expr::Imm(0xFFFFFFFFFFFFFFFF)),
                Box::new(Expr::Imm(0x00000000FFFFFFFF)),
            ),
        }];
